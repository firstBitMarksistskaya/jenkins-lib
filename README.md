...
## Ссылка на экран агента

Если у узла Jenkins задана переменная окружения `AGENT_SCREEN_URL` (*Настройка узла → Свойства узла → Переменные окружения*), в начале этапов, выполняемых на агенте 1С (создание ИБ, распаковка ИБ перед BDD, синтаксическим контролем, дымовыми тестами и YAXUnit), ссылка выводится в лог сборки.

При установленном плагине [Badge](https://plugins.jenkins.io/badge/) ссылка дополнительно добавляется бейджем на страницу сборки — один бейдж на узел. Без плагина шаг ограничивается записью в лог.

Удобно для наблюдения за тестами с графическим интерфейсом: например, в переменной можно указать адрес noVNC, подключённого к виртуальному дисплею агента.
