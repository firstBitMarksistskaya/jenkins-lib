...
* Инициализация:
  ...
  * Если формат исходников (`sourceFormat`) выбран как `edt`, то (`initInfobase` -> `initMethod`) должен быть установлен `fromSource`, т.к. Инициализация базы не зависит от формата хранения исходников. 
